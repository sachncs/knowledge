...
class BundleSerializer:
    ...
    def __init__(self, path_map: dict[str, str] | None = None) -> None:
        ...
        Raises:
            ValueError: If any path in *path_map* is absolute, contains
                parent-directory references (``..``), or resolves to a
                location outside the bundle root.  This guards against
                path traversal via a malicious tag-to-path mapping.
        """
        validated: dict[str, str] = {}
        for tag, subdir in (path_map or {}).items():
            self._validate_subdir(subdir)
            validated[tag] = subdir
        self.path_map = validated

    @staticmethod
    def _validate_subdir(subdir: str) -> None:
        """Reject subdirectory paths that could escape the bundle root.

        Args:
            subdir: Candidate subdirectory path.

        Raises:
            ValueError: If *subdir* is absolute, contains ``..`` segments,
                or normalises to a path that escapes the root.
        """
        if os.path.isabs(subdir):
            raise ValueError(f"path_map entries must be relative paths, got absolute: {subdir!r}")
        parts = subdir.replace("\\", "/").split("/")
        if any(p == ".." for p in parts):
            raise ValueError(f"path_map entries must not contain '..' segments, got: {subdir!r}")
        if any(p == "" for p in parts[:-1]):
            raise ValueError(f"path_map entries must not contain empty segments, got: {subdir!r}")
    ...
    def serialize(self, graph: KnowledgeGraph, output_dir: str) -> int:
        ...
        5. Removes stale ``.md`` files in directories the serializer
           owns.  Non-Markdown files (assets, images, user-authored
        ...
        for subdir, entries in sorted(path_groups.items()):
            target = os.path.join(output_dir_abs, subdir)
            os.makedirs(target, exist_ok=True)
            for cid, _, _ in entries:
                filepath = self.write_concept(target, graph.concepts[cid])
                written_files.add(os.path.normpath(filepath))
                written += 1

        for cid, _, _ in flat_concepts:
            filepath = self.write_concept(output_dir_abs, graph.concepts[cid])
            written_files.add(os.path.normpath(filepath))
            written += 1
        ...
        filepath = self.write_index(output_dir_abs, "Knowledge Bundle", "bundle", root_entries)
        written_files.add(os.path.normpath(filepath))

        # remove stale .md files only in directories the serializer owns,
        # and never touch non-.md files (assets, user-authored content).
        managed_dirs = {output_dir_abs}
        for subdir in all_dirs:
            managed_dirs.add(os.path.normpath(os.path.join(output_dir_abs, subdir)))
        for dirpath, _, filenames in os.walk(output_dir_abs):
            if os.path.normpath(dirpath) not in managed_dirs:
                continue
            for f in filenames:
