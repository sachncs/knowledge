...
* **Graceful degradation** — a failing LLM call for one section does not
  ...
  level and skipped.  All ``litellm`` exceptions are caught uniformly
  so behaviour is consistent across providers.
"""
...
Return exactly this JSON shape:
{
    ...
    def split_sections(self, text: str) -> list[tuple[str, str, int]]:
        ...
        1. :meth:`split_html_headings` — looks for ``<h1>``–``<h6>``
           tags.  HTML comments (``<!-- ... -->``) are stripped before
    ...
    def split_markdown_headings(text: str) -> list[tuple[str, str, int]]:
        ...
        (e.g. ``## Installation ##`` → ``"Installation"``).  Setext-style
        headings (``Foo`` followed by ``=====`` or ``-----``) are also
        recognised and reported as level 1 or level 2 headings.

        The body of a section runs from the line after the heading to
    ...
    def extract_section(self, heading: str, content: str, level: int) -> Concept | None:
        ...
        try:
            ...
            logger.warning("LLM error for section '%s'", heading, exc_info=True)
            return None
        except Exception:
            logger.warning("Unexpected error calling LLM for section '%s'", heading, exc_info=True)
            return None
...
def strip_json_fence(text: str) -> str:
    ...
    If the model emits prose *after* the closing fence (a common
    failure mode), the prose is also removed so the JSON object can be
    parsed.
    ...
    Returns:
        The text with outer code fences and any trailing prose removed,
        if present.
    """
    text = text.strip()
    if text.startswith("```"):
