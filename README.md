...
[**OKF v0.1**](https://sachncs.github.io/knowledge/decisions/0002-okf-v0-1-bundle-format/)
directory bundle.
...
- **LLM-Powered Extraction** — Routes any provider that
  [litellm](https://litellm.ai) supports (OpenAI, Anthropic, Ollama,
  vLLM, Google Gemini, AWS Bedrock, Azure OpenAI, and others) through
  a single interface.
...
| `Knowledge.update(source, bundle_dir)` | method | **Destructive.** Re-extracts and *overwrites* every file in the bundle; manual edits are lost. |
| `Knowledge.remove(concept_ids, bundle_dir)` | method | Delete concepts by ID from a bundle. Unknown IDs are reported via `Knowledge.remove_with_unknowns()`. |
