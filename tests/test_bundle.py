...
class TestBundleSerializer:
    ...
    def test_yaml_escape_round_trip(self) -> None:
        inputs = [
            ...
            "tab\there",
            "\x00null",
            # Inputs that previously broke round-trip (issue #39)
            "\\n",
            "\\r",
            "\\t",
            "mix\\nstuff",
        ...
        for raw in inputs:
            ...
            # Verify the actual round-trip via yaml_unescape
            from knowledge.llm.manager import yaml_unescape

            assert yaml_unescape(escaped) == raw, (
                f"round-trip failed for {raw!r}: escaped={escaped!r}"
            )

    def test_path_map_rejects_absolute_paths(self) -> None:
        """Absolute paths in path_map must be rejected (issue #40)."""
        with pytest.raises(ValueError, match="absolute"):
            BundleSerializer(path_map={"tag": "/etc/passwd"})

    def test_path_map_rejects_parent_traversal(self) -> None:
        """Paths containing '..' must be rejected (issue #40)."""
        with pytest.raises(ValueError, match=r"\.\."):
            BundleSerializer(path_map={"tag": "../../escape"})

    def test_path_map_rejects_empty_segments(self) -> None:
        """Empty path segments (leading/trailing slashes) must be rejected."""
        with pytest.raises(ValueError, match="empty"):
            BundleSerializer(path_map={"tag": "foo//bar"})

    def test_serialize_does_not_traverse_above_output(self) -> None:
        """Even with a benign path_map, files stay inside output."""
        with tempfile.TemporaryDirectory() as tmpdir:
            output = os.path.join(tmpdir, "bundle")
            os.makedirs(output)
            BundleSerializer(path_map={"a": "subdir"}).serialize(KnowledgeGraph(), output)
